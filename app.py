...
# ── FIX 3: Track last rendered emotion so we only re-render ──
# the right-panel widgets when the emotion actually changes.
_last_rendered_emotion = None
_last_rendered_scores  = None
_last_rendered_log_len = 0
...
# ─── DISPLAY FRAME SIZE ───────────────────────────────────────
# FIX 4: Downscale frames before sending to Streamlit.
# The model already works on 48px; there's no reason to push
# a full 1080p frame over the wire every tick.
DISPLAY_WIDTH = 640  # pixels — adjust to taste
...
if not st.session_state.running:
    ...
    if not cap.isOpened():
        ...
        # Hint to camera driver — reduces decode cost on high-res webcams
        ...
        # Seed the section headers once so placeholders aren't blank
        feed_label_ph.markdown('<div class="section-label">Session feed</div>', unsafe_allow_html=True)
        ...
        while st.session_state.running:
            ...
            if results:
                new_emo   = results[0]['emotion']
                new_conf  = results[0]['confidence']
                new_scores= results[0]['scores']
                st.session_state.history.append(new_emo)
                if len(st.session_state.history) > 100:
                    st.session_state.history.pop(0)
                if new_emo != last_emo:
                    emotion_changed = True
                last_emo, last_conf, last_scores = new_emo, new_conf, new_scores
            else:
                gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                new_emo, new_conf, new_scores = predict_emotion(gray)
                if new_emo != last_emo:
                    emotion_changed = True
                last_emo, last_conf, last_scores = new_emo, new_conf, new_scores

            # ── Always update: camera feed + frame counter ────
            # Resize annotated frame before handing to Streamlit
            h, w = annotated.shape[:2]
            if w > DISPLAY_WIDTH:
                scale      = DISPLAY_WIDTH / w
                display_h  = int(h * scale)
                display_fr = cv2.resize(annotated, (DISPLAY_WIDTH, display_h),
                                        interpolation=cv2.INTER_LINEAR)
            else:
                display_fr = annotated

            cam_ph.image(cv2.cvtColor(display_fr, cv2.COLOR_BGR2RGB),
                         channels="RGB", use_container_width=True)

            face_note = (f"{len(results)} face detected"
                         if results else "no face detected — reading full frame")
            feed_meta_ph.markdown(
                f'<div class="feed-meta">Frame {st.session_state.frames} · {face_note}</div>',
                unsafe_allow_html=True)

            # ── Only re-render right panel when emotion changes ─
            log_grew = len(st.session_state.history) != _last_rendered_log_len
            if emotion_changed or log_grew:
                render_reading(last_emo, last_conf)
                render_stamp(last_emo)
                render_probs(last_scores)
                render_log(st.session_state.history)
                _last_rendered_emotion = last_emo
                _last_rendered_log_len = len(st.session_state.history)
